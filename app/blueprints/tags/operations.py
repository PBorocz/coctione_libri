...
def remove_tag(tag: str) -> None:
    ...
    Documents.objects(tags__in=[tag]).update(pull__tags=tag)


def update_tag(old: str, new: str) -> None:
    ...
    Documents.objects(id__in=ids).update(pull__tags=old)
    Documents.objects(id__in=ids).update(push__tags=new)
